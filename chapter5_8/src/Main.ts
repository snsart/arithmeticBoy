class Main extends egret.DisplayObjectContainer {
    ...
    private setup(){
        ...
        listener.BeginContact=function(cp){
            let f1=cp.GetFixtureA();
            let f2=cp.GetFixtureB();
            let b1=f1.GetBody();
            let b2=f2.GetBody();
            let p1=b2.GetUserData();
        }
    }
    ...
    private loop(e:egret.Event){
        ...
        for(let box of this.boxes){
            let wind=new Box2D.Common.Math.b2Vec2(0.2,0);
            box.applyForce(wind);
            box.display();
        }
...
class Listener implements Box2D.Dynamics.b2ContactListener{
