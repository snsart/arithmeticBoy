...
var Vehicle = (function (_super) {
    ...
    */
    ...
    Vehicle.prototype.follow = function (p) {
        ...
        //第二步：在路径上寻找法线交点
        var normalPoint = this.getNormalPoint(predictLoc, p.start, p.end);
        //第三步：沿着路径前进一段距离，将其设为目标
        var dir = Vector2D.sub(p.end, p.start);
        dir.normalize();
        dir.mult(10);
        var target = Vector2D.add(normalPoint, dir);
        //第四步：如果脱离了路径，就寻找之前设定的目标，然后回归路径
        var distance = Vector2D.dist(normalPoint, predictLoc);
